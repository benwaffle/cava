...
void *input_alsa(void *data) {
    ...
    if ((err = snd_pcm_open(&handle, audio->source, SND_PCM_STREAM_CAPTURE, 0) <
               0)) {
...
/* EPIPE means overrun */
        ...
        for (i = 0; i < size; i = i + (ladj)*2) {
            // first channel
            tempr =
                ((buffer[i + (radj)-1] << 2)); // using the 10 upper bits this
                                               // would give me a vert res of
                                               // 1024, enough...
            ...
            if (audio->channels == 1)
                audio->audio_out_l[o] = (tempr + templ) / 2;
