...
int draw_terminal_noncurses(int tty, int h, int w, int bars, int bw, int bs,
    ...
    for (n = h - 2; n >= 0; n--) {
        ...
        for (o = 0; o < bars; o++) {
            // output: draw and move to another one, check whether we're not too
            // far
            ...
            if (f[o] != flastd[o]) { // change?
                ...
                if (c < 1) {
                    ...
                    if (tty)
                        printf("%d",
                               ttybarstring[c - 1]); // fractioned block tty
                    else
