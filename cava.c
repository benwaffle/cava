...
double smoothDef[64] = {0.8, 0.8, 1,   1,   0.8, 0.8, 1,   0.8, 0.8, 1,   1,
                        0.8, 1,   1,   0.8, 0.6, 0.6, 0.7, 0.8, 0.8, 0.8, 0.8,
...
void load_config(char configPath[255]) {
    ...
    if (configPath[0] == '\0') {
        ...
        if (configHome != NULL) {
            ...
            if (configHome != NULL) {
                sprintf(configPath, "%s/%s/%s/", configHome, ".config",
                        PACKAGE);
            } else {
...
// setting pulse to defaualt if supported
...
#ifndef NCURSES
    outputMethod =
        (char *)iniparser_getstring(ini, "output:method", "noncurses");
#endif
    ...
    raw_target =
        (char *)iniparser_getstring(ini, "output:raw_target", "/dev/stdout");
    data_format =
        (char *)iniparser_getstring(ini, "output:data_format", "binary");
    ...
    if (strcmp(inputMethod, "alsa") == 0) {
        im = 1;
        audio.source =
            (char *)iniparser_getstring(ini, "input:source", "hw:Loopback,1");
    }
    if (strcmp(inputMethod, "fifo") == 0) {
        im = 2;
        audio.source =
            (char *)iniparser_getstring(ini, "input:source", "/tmp/mpd.fifo");
    }
...
int validate_color(char *checkColor, int om) {
    ...
    if (checkColor[0] == '#' && strlen(checkColor) == 7) {
        ...
        // colour
        // instead of hex colours.
        if (om != 1 && om != 2) {
            fprintf(stderr,
                    "Only 'ncurses' output method supports HTML colors. "
                    "Please change the colours or the output method.\n");
            exit(EXIT_FAILURE);
        }
        // 0 to 9 and a to f
        for (int i = 1; checkColor[i]; ++i) {
            if (!isdigit(checkColor[i])) {
                if (tolower(checkColor[i]) >= 'a' &&
                    tolower(checkColor[i]) <= 'f') {
                    validColor = 1;
                } else {
                    validColor = 0;
                    break;
...
void validate_config() {
...
#ifndef ALSA
        fprintf(stderr,
                "cava was built without alsa support, install alsa dev "
...
#ifndef PULSE
        fprintf(stderr,
                "cava was built without pulseaudio support, install "
                "pulseaudio dev files and run make clean && ./configure && "
                "make again\n");
        exit(EXIT_FAILURE);
#endif
    }
...
#ifndef NCURSES
        fprintf(stderr,
                "cava was built without ncurses support, install ncursesw "
                "dev files and run make clean && ./configure && make "
                "again\n");
        exit(EXIT_FAILURE);
#endif
    }
    ...
    if (strcmp(outputMethod, "raw") == 0) { // raw:
        ...
        if (strcmp(data_format, "binary") == 0) {
            ...
            if (bit_format != 16 && bit_format != 16) {
                fprintf(stderr,
                        "bit format  %d is not supported, supported data "
                        "formats are: '8' and '16'\n",
                        bit_format);
                exit(EXIT_FAILURE);
            }
            ...
            is_bin = 0;
            if (ascii_range < 1) {
                fprintf(stderr, "ascii max value must be a positive integer\n");
                exit(EXIT_FAILURE);
            }
        } else {
            fprintf(stderr,
...
#ifndef NCURSES
        fprintf(stderr,
                "output method %s is not supported, supported methods are: "
                "'noncurses'\n",
                outputMethod);
        exit(EXIT_FAILURE);
#endif
    ...
    if (strcmp(style, "mono") == 0)
        stereo = 0;
    if (strcmp(style, "stereo") == 0)
        stereo = 1;
    if (stereo == -1) {
        fprintf(stderr,
                "output style %s is not supported, supported styles are: "
                "'mono' and 'stereo'\n",
                style);
        exit(EXIT_FAILURE);
    }
    ...
    if (strcmp(modeString, "normal") == 0)
        mode = 1;
    if (strcmp(modeString, "scientific") == 0)
        mode = 2;
    if (strcmp(modeString, "waves") == 0)
        mode = 3;
    if (mode == 0) {
        fprintf(stderr,
                "smoothing mode %s is not supported, supported modes are: "
                "'normal', 'scientific', 'waves'\n",
                modeString);
        exit(EXIT_FAILURE);
    }
    ...
    if (!validate_color(color, om)) {
        fprintf(stderr,
                "The value for 'foreground' is invalid. It can be either "
                "one of the 7 named colors or a HTML color of the form "
                "'#xxxxxx'.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (!validate_color(bcolor, om)) {
        fprintf(stderr,
                "The value for 'background' is invalid. It can be either "
                "one of the 7 named colors or a HTML color of the form "
                "'#xxxxxx'.\n");
        exit(EXIT_FAILURE);
    }

    // In case color is not html format set bgcol and col to predefinedint
    // values
    ...
    // validate: background color
    if (strcmp(bcolor, "black") == 0)
        bgcol = 0;
    if (strcmp(bcolor, "red") == 0)
        bgcol = 1;
    if (strcmp(bcolor, "green") == 0)
        bgcol = 2;
    if (strcmp(bcolor, "yellow") == 0)
        bgcol = 3;
    if (strcmp(bcolor, "blue") == 0)
        bgcol = 4;
    if (strcmp(bcolor, "magenta") == 0)
        bgcol = 5;
    if (strcmp(bcolor, "cyan") == 0)
        bgcol = 6;
    if (strcmp(bcolor, "white") == 0)
        bgcol = 7;
    // default if invalid
    ...
    if (lowcf > highcf) {
        fprintf(stderr,
                "lower cutoff frequency can't be higher than higher cutoff "
                "frequency\n");
        exit(EXIT_FAILURE);
    }
...
static bool is_loop_device_for_sure(const char *text) {
    ...
    return strncmp(text, LOOPBACK_DEVICE_PREFIX,
                   strlen(LOOPBACK_DEVICE_PREFIX)) == 0;
}
...
int *separate_freq_bands(fftw_complex out[M / 2 + 1][2], int bars, int lcf[200],
                         int hcf[200], float k[200], int channel) {
    int o, i;
    float peak[201];
    static int fl[200];
    static int fr[200];
    ...
    for (o = 0; o < bars; o++) {
        ...
        for (i = lcf[o]; i <= hcf[o]; i++) {
            y[i] = pow(pow(*out[i][0], 2) + pow(*out[i][1], 2),
                       0.5); // getting r of compex
            peak[o] += y[i]; // adding upp band
        }

        peak[o] = peak[o] / (hcf[o] - lcf[o] + 1); // getting average
        temp = peak[o] * k[o] *
               sens; // multiplying with k and adjusting to sens settings
        if (temp <= ignore)
            temp = 0;
        if (channel == 1)
            fl[o] = temp;
...
as of 0.4.0 all options are specified in config file, see in '/home/username/.config/cava/' \n";
    ...
    while (1) {
        ...
        if (om != 4) {
            // Check if we're running in a Virtual console todo: replace virtual
            // console with terminal emulator
            inAtty = 0;
        ...
        if (im == 1) {
            if (is_loop_device_for_sure(audio.source)) {
                if (directory_exists("/sys/")) {
                    if (!directory_exists("/sys/module/snd_aloop/")) {
                        fprintf(
                            stderr,
                            "Linux kernel module \"snd_aloop\" does not seem "
                            "to be loaded.\n"
            ...
            thr_id =
                pthread_create(&p_thread, NULL, input_alsa,
            ...
            while (audio.format == -1 || audio.rate == 0) {
                ...
                if (n > 2000) {
                    ...
                    cleanup();
                    fprintf(stderr,
                            "could not get rate and/or format, problems with "
                            "audio thread? quiting...\n");
#endif
                    exit(EXIT_FAILURE);
                }
            }
        ...
        if (im == 2) {
            thr_id =
                pthread_create(&p_thread, NULL, input_fifo,
        ...
        if (im == 3) {
            thr_id =
                pthread_create(&p_thread, NULL, input_pulse,
                               (void *)&audio); // starting fifomusic listener
            audio.rate = 44100;
        }
#endif
        ...
        pl = fftw_plan_dft_r2c_1d(M, inl, *outl,
                                  FFTW_MEASURE); // planning to rock
        ...
        while (
            !reloadConf) { // jumbing back to this loop means that you resized
                           // the screen
            for (i = 0; i < 200; i++) {
                flast[i] = 0;
                flastd[i] = 0;
            ...
            if (om == 4) {
                if (strcmp(raw_target, "/dev/stdout") != 0) {
                    ...
                    if (access(raw_target, F_OK) != -1) {
                        fptest = open(raw_target, O_RDONLY | O_NONBLOCK,
                                      0644); // testopening in case it's a fifo
                        if (fptest == -1) {
                        ...
                        fptest = open(
                            raw_target, O_RDONLY | O_NONBLOCK,
                            0644); // fifo needs to be open for reading in order
                                   // to write to it
                    }

                    fp =
                        open(raw_target, O_WRONLY | O_NONBLOCK | O_CREAT, 0644);
                    if (fp == -1) {
                        printf("could not open file %s for writing\n",
                               raw_target);
                        exit(1);
            ...
            if (fixedbars) {
                autobars = 0;
                if (fixedbars * bw + fixedbars * bs - bs > w)
                    autobars = 1;
            }
            ...
            if (bars < 1)
                bars = 1; // must have at least 1 bar;
            ...
            g = gravity * ((float)height / 270) *
                pow((60 / (float)framerate), 2.5);
            ...
            if (om == 3)
                init_terminal_noncurses(col, bgcol, w, h, bw);

            if (stereo)
                bars =
                    bars / 2; // in stereo onle half number of bars per channel
            ...
            for (n = 0; n < bars + 1; n++) {
                fc[n] = highcf *
                        pow(10, freqconst * (-1) +
                                    ((((float)n + 1) / ((float)bars + 1)) *
                                     freqconst)); // decided to cut it at 10k,
                // little interesting to hear
                // above
                fre[n] = fc[n] / (audio.rate / 2); // remember nyquist!, pr my
                // calculations this should be rate/2
                // and  nyquist freq in M/2 but
                // testing shows it is not... or
                // maybe the nq freq is in M/4
                lcf[n] =
                    fre[n] * (M / 4); // lfc stores the lower cut frequency foo
                                      // each bar in the fft out buffer
                if (n != 0) {
                    hcf[n - 1] = lcf[n] - 1;
                    if (lcf[n] <= lcf[n - 1])
                        lcf[n] = lcf[n - 1] +
                                 1; // pushing the spectrum up if the expe
                                    // function gets "clumped"
                    hcf[n - 1] = lcf[n] - 1;
                }
            ...
            for (n = 0; n < bars; n++)
                k[n] = pow(fc[n], 0.85) * ((float)height / (M * 4000)) *
                       smooth[(int)floor(((double)n) * smh)];
...
// if (cont == 0) break;
                ...
                // process: if input was present for the last 5 seconds apply
                // FFT to it
                if (sleep < framerate * 5) {
                    ...
                    if (stereo) {
                        ...
                        fl =
                            separate_freq_bands(outl, bars / 2, lcf, hcf, k, 1);
                        fr =
                            separate_freq_bands(outr, bars / 2, lcf, hcf, k, 2);
                    } else {
                        fftw_execute(pl);
...
#ifdef DEBUG
                    printw(
                        "no sound detected for 3 sec, going to sleep mode\n");
#endif
...
// output: draw processed input
                ...
                case 1:
#ifdef NCURSES
                    rc = draw_terminal_ncurses(inAtty, h, w, bars, bw, bs, rest,
                                               f, flastd);
                    break;
#endif
                case 2:
...
#endif
                case 3:
                    rc = draw_terminal_noncurses(inAtty, h, w, bars, bw, bs,
                                                 rest, f, flastd);
                    break;
                case 4:
                    rc = print_raw_out(bars, fp, is_bin, bit_format,
                                       ascii_range, bar_delim, frame_delim, f);
                    break;
                }

                if (rc == -1)
                    resizeTerminal =
                        TRUE; // terminal has been resized breaking to
                ...
                if (framerate <= 1) {
                    ...
                    req.tv_sec = 0;
                    req.tv_nsec = (1 / (float)framerate) *
                                  1000000000; // sleeping for set us
                }
